...
class Distiller:
    def __init__(
        self, params: dict, dataloader: torch.utils.data.DataLoader, token_probs: torch.tensor, student: nn.Module, teacher: nn.Module
    ):
        ...
        assert params.grad_acc_steps >= 1
        self.num_steps_epoch = len(self.dataloader)
        num_train_optimization_steps = (
            int(self.num_steps_epoch / params.grad_acc_steps * params.n_epochs) + 1
        )
        ...
        for _ in range(self.params.n_epochs):
            ...
            for batch in iter_bar:
                ...
                if self.mlm:
                    ...
                    print('\n\n\n', torch.max(input_ids))
                    #print(input_ids)
                    #print(torch.max(input_ids), torch.max(position_ids), torch.max(token_type_ids), torch.max(label_ids))
                print('\n\n\n', torch.max(input_ids))
                #print(batch)
                #print("about to step\n\n\n\n")
                self.step(input_ids=input_ids, position_ids = position_ids, token_type_ids= token_type_ids, lm_labels=label_ids)
    ...
    def step(self, input_ids: torch.tensor, position_ids: torch.tensor, token_type_ids: torch.tensor, lm_labels: torch.tensor):
        """
        ...
        if self.mlm:
            ...
            #print(self.student, s)
            stu_out = self.student(
                input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, 
                return_dict=True, output_hidden_states=True
            )  # (bs, seq_length, voc_size)
            s_logits = stu_out.logits
            # log chatlogs
            #preds = greedy(s_logits, tokenizer)
            #f1, prec, recall = f1_per_batch(preds, label_ids, tokenizer, args, n_steps)
            #rouge_score = rouge_per_batch(preds, label_ids, tokenizer, args, n_steps)

            s_hidden_states = stu_out.hidden_states
            #print('shapessssss', s_logits.size(), s_hidden_states.size(), '\n\n\n')
            with torch.no_grad():
                tea_out = self.teacher(
                    input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, 
                    return_dict=True, output_hidden_states=True
                )  # (bs, seq_length, voc_size)
                t_logits = tea_out.logits
                t_hidden_states = tea_out.hidden_states
        ...
        s_logits_slct = s_logits#_slct.view(-1, s_logits.size(-1))  # (bs * seq_length, voc_size) modulo the 1s in mask
        #t_logits_slct = torch.masked_select(t_logits, mask)  # (bs * seq_length * voc_size) modulo the 1s in mask
        t_logits_slct = t_logits#_slct.view(-1, s_logits.size(-1))  # (bs * seq_length, voc_size) modulo the 1s in mask
        ...
        if self.alpha_cos > 0.0:
            ...
            s_hidden_states_slct = s_hidden_states#torch.masked_select(s_hidden_states, mask)  # (bs * seq_length * dim)
            s_hidden_states_slct = s_hidden_states_slct.view(-1, dim)  # (bs * seq_length, dim)
            t_hidden_states_slct = t_hidden_states#torch.masked_select(t_hidden_states, mask)  # (bs * seq_length * dim)
    ...
    def optimize(self, loss):
        ...
        if self.multi_gpu:
            loss = loss.mean()
        if self.params.grad_acc_steps > 1:
            loss = loss / self.params.grad_acc_steps
        ...
        self.iter()
        if self.n_iter % self.params.grad_acc_steps == 0:
            if self.fp16:
